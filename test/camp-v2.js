...
test('three-stage shelter upgrades preserve instance identity and spend atomically',()=>{const s=C.fresh();const r=C.command(s,{kind:'upgrade',id:'o1',x:3,y:2});assert(r.ok);assert.equal(r.save.objects[0].type,'canvas');assert.equal(r.save.gems,75);assert.equal(r.save.wood,5);assert.equal(s.objects[0].type,'tent');});
...
(async()=>{
  ...
  const source=fs.readFileSync(path.join(root,'index.html'),'utf8').replace('<script src="camp-v2.js"></script>',()=>`<script>${fs.readFileSync(path.join(root,'camp-v2.js'),'utf8')}</script>`);
