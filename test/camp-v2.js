...
test('early shelter renovation preserves instance identity and spends atomically',()=>{const s=C.syncProgress(C.fresh(),3);const r=C.command(s,{kind:'upgrade',id:'o1',x:3,y:2});assert(r.ok);assert.equal(r.save.objects[0].type,'trailtent');assert.equal(r.save.gems,108);assert.equal(r.save.wood,8);assert.equal(s.objects[0].type,'tent');});
