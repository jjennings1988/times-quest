...
test('the prior 2.5D save migrates without changing ownership or object coordinates',()=>{
  ...
  const next=C.migrateSave(old);assert.equal(next.v,3);assert(C.validSave(next));assert.equal(next.gems,73);assert.equal(next.wood,4);assert.deepEqual(next.objects,old.objects);assert.deepEqual(next.inventory,old.inventory);assert.deepEqual(next.projects,old.projects);assert.deepEqual(old,before);
});
...
test('forest trunks block walking and out-of-world destinations are refused',()=>{const t=C.world.forest.find(t=>C.world.inside(Math.floor(t.x),Math.floor(t.z)));assert(C.world.blocked(Math.floor(t.x),Math.floor(t.z)));assert.equal(C.route(C.fresh(),{x:6,y:7},{x:87,y:20}),null);});
