...
function renderZeroLesson(){
  ...
  let task='';
  if(step==='inspect')task=`<h2>What is inside Poof’s baskets?</h2><p>There are <b>3 baskets</b>. Open each one to find how many supplies it holds.</p>${zeroBaskets(3,0,{inspect:true})}<p class="zero-equation">3 groups × ${opened.length===3?'0':'?'} in each = <strong>${opened.length===3?'0':'?'}</strong></p><p role="status">${opened.length===3?'All three baskets are empty. Three groups of nothing still make zero.':`${opened.length} of 3 baskets inspected`}</p><button class="btn gold" onclick="advanceZeroLesson()" ${opened.length<3?'disabled':''}>Now try a different arrangement</button>`;
