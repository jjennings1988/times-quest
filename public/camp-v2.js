...
(function (root) {
  ...
  // Every blueprint, including homes, lives in camp-content.js (one owner for costs, gates and upgrade chains).
  ...
  function validSave(s){
    ...
    if(s.v===3){
      ...
      if(s.challenge&&(!['fish','stone','tree','land','order','bake','mill'].includes(s.challenge.kind)||!Array.isArray(s.challenge.questions)||!(SINGLE.includes(s.challenge.kind)?[1]:[2,3]).includes(s.challenge.questions.length)||!Number.isInteger(s.challenge.index)||s.challenge.index<0||s.challenge.index>=s.challenge.questions.length||s.challenge.questions.some(q=>!Number.isInteger(q.a)||!Number.isInteger(q.b)||q.a<0||q.a>12||q.b<0||q.b>12)))return false;
    }
    if(!Object.entries(s.inventory).every(([k,v])=>catalog[k]&&Number.isInteger(v)&&v>=0&&v<=10000))return false;
    for(const k of ['solved','factsBuilt','requestsDone'])if(s[k]!==undefined&&(!Array.isArray(s[k])||s[k].length>200||s[k].some(v=>typeof v!=='string'||v.length>40)))return false;
    if(s.campName!==undefined&&(typeof s.campName!=='string'||!validName(s.campName)))return false;
    ...
    if(s.arrivals!==undefined&&(!Array.isArray(s.arrivals)||s.arrivals.length>60||s.arrivals.some(a=>!a||!catalog[a.type]||!['kit','keepsake'].includes(a.kind))))return false;
  ...
  function placementReason(s,type,x,y,r=0,ignore=null,checkAccess=true){
    ...
    // A garden bed and a building never share ground.
    if(type==='plot'&&s.objects.some(o=>o.id!==ignore&&catalog[o.type].layer==='solid'&&overlaps(box,{...o,...footprint(o.type,o.r)})))return 'Plant around buildings, not under them';
    if(d.layer==='solid'&&s.objects.some(o=>o.id!==ignore&&o.type==='plot'&&overlaps(box,{...o,w:1,h:1})))return 'Leave your garden bed clear';
  ...
  function unlockReason(s,type){const d=catalog[type];if(d?.kitOnly&&!((s.inventory?.[type]||0)>0))return 'Restore this realm to earn its keepsake';return (s.mastered||0)<(d?.need||0)?'Complete '+d.need+' Realm Challenges to unlock this blueprint':'';}
  function syncProgress(save,mastered,tester=false){let s=migrateSave(save);if(!s)return save;s=clone(s);s.mastered=Math.max(s.mastered,Math.min(13,Math.max(0,Math.floor(mastered||0))));s=grantKits(s);for(const z of content.zones)if(z.need===0&&!s.land.includes(z.id))s.land.push(z.id);if(tester&&s.qaProvision!==2){s=provision(s);s.qaProvision=2;}if(tester)for(const z of content.zones)if(!s.land.includes(z.id))s.land.push(z.id);return s;}
  /* Each restored family's signature build arrives once as a ready-to-place kit (retroactive for earlier milestones). */
  function grantKits(save){const s=save,given=s.kitsGranted||(s.kitsGranted=[]);s.arrivals=s.arrivals||[];
    if(!s.orchardKit){s.orchardKit=1;for(const [type,count] of [['fruittree',1],['critterhome',1]]){s.inventory[type]=Math.min(10000,(s.inventory[type]||0)+count);s.arrivals.push({kind:'kit',type,count,milestone:0});}}
    if(!s.gardenKit){s.gardenKit=1;for(const [type,count] of [['plot',12],['deck',6]]){s.inventory[type]=Math.min(10000,(s.inventory[type]||0)+count);s.arrivals.push({kind:'kit',type,count,milestone:0});}}
    for(let n=1;n<=s.mastered;n++){if(given.includes(n))continue;given.push(n);for(const [type,count] of Object.entries(content.kits[n]||{})){if(!catalog[type])continue;s.inventory[type]=Math.min(10000,(s.inventory[type]||0)+count);s.arrivals.push({kind:'kit',type,count,milestone:n});}}
    s.arrivals=s.arrivals.slice(-60);return s;}
  /* A keepsake finds its own place in the trophy garden around the Story Stones; if no spot is free it waits in the backpack. */
  function autoPlace(save,type,preferred=0){if(!validSave(save)||!((save.inventory[type]||0)>0))return {save,placed:null};
    const spots=content.trophySpots,order=[...spots.slice(preferred),...spots.slice(0,preferred)];
    for(const [x,y] of order){const r=command(save,{kind:'place',type,x,y});if(r.ok){const s=r.save;s.arrivals=[...(s.arrivals||[]),{kind:'keepsake',type,x,y}].slice(-60);return {save:s,placed:{x,y}};}}
    const s=clone(save);s.arrivals=[...(s.arrivals||[]),{kind:'keepsake',type}].slice(-60);return {save:s,placed:null};}
  ...
  function command(save,cmd,context={}){
    if(!validSave(save))return {ok:false,message:'This camp needs recovery. Open Camp help; your save has been kept.'};
    ...
    if(['place','move','upgrade'].includes(cmd.kind)){
      ...
      if(!o)return {ok:false,message:'Select an object'};o.color=Number.isInteger(cmd.color)&&cmd.color>=0&&cmd.color<5?cmd.color:((o.color||0)+1)%5;message='A new colour, just for your camp.';
      ...
      else if(kind==='stone'){if(s.mastered<3)return {ok:false,message:'Restore 3 realms to open the river stone bar'};if(s.casts<1)return {ok:false,message:'A learning adventure brings three more river trips'};if(Math.hypot(s.explorer.x-14,s.explorer.y-8)>2)return {ok:false,message:'Walk to the river first'};}
      else if(kind==='bake'){if(!near(s.explorer,'bakery'))return {ok:false,message:'Visit Bram at the bakery'};if((s.bakes??1)<1)return {ok:false,message:'Bram bakes again after your next learning adventure'};}
      else if(kind==='mill'){if(!near(s.explorer,'mill'))return {ok:false,message:'Visit Millie at the watermill'};if((s.mills??1)<1)return {ok:false,message:'The wheel turns again after your next learning adventure'};}
      else if(kind==='order'){if(Math.hypot(s.explorer.x-26,s.explorer.y-4)>2)return {ok:false,message:'Visit Mara’s store across the bridge'};if((s.orders??2)<1)return {ok:false,message:'Mara has more orders after your next learning adventure'};}
      ...
      // An order is one fact as equal groups; Mara prefers crates that are neither empty nor missing.
      if(SINGLE.includes(kind)&&Array.isArray(cmd.questions)&&cmd.questions.length)cmd={...cmd,questions:[cmd.questions.find(q=>q&&q.a>0&&q.b>0)||cmd.questions[0]]};
      if(!Array.isArray(cmd.questions)||!(SINGLE.includes(kind)?[1]:[2,3]).includes(cmd.questions.length)||cmd.questions.some(q=>!Number.isInteger(q.a)||!Number.isInteger(q.b)||q.a<0||q.a>12||q.b<0||q.b>12))return {ok:false,message:'Review questions are unavailable'};
      s.challenge={kind,target:target||null,questions:cmd.questions.map(q=>({a:q.a,b:q.b})),index:0,retry:false,feedback:false};message=kind==='order'?'One order for Mara, at your pace. Mistakes are for learning.':kind==='bake'?'One baking order for Bram. Take your time.':kind==='mill'?'Share the logs fairly. Take your time.':(cmd.questions.length===2?'Two':'Three')+' review facts, at your pace. Mistakes are for learning.';
    }else if(cmd.kind==='solve-pattern'){
      const p=patterns(s).find(p=>p.key===cmd.key);if(!p||!mathable(p))return {ok:false,message:'That bed has changed. Build it again to count it.'};
      if(String(cmd.answer).trim()===''||!Number.isFinite(Number(cmd.answer)))return {ok:false,message:'Enter an answer'};
      const correct=Number(cmd.answer)===p.count;reviewEvent={a:p.rows,b:p.cols,correct,firstTry:s.patternTry!==p.key};
      if(!correct){s.patternTry=p.key;message=`Count it by rows: ${p.rows} rows of ${p.cols}. Try again.`;}
      else{s.patternTry=null;s.solved=[...(s.solved||[]).filter(k=>k!==p.key),p.key].slice(-60);if(p.type==='plot'&&Number.isInteger(cmd.day))s.beds={...(s.beds||{}),[p.key]:cmd.day};const fact=`${p.type}:${Math.min(p.rows,p.cols)}x${Math.max(p.rows,p.cols)}`;
        message=`${p.rows} × ${p.cols} = ${p.count}!`;if(!(s.factsBuilt||[]).includes(fact)){s.factsBuilt=[...(s.factsBuilt||[]),fact].slice(-200);s.gems+=3;message+=' A new fact for your camp · +3 gems';}}
    }else if(cmd.kind==='harvest'){
      // Gather whatever is ready: one bed, one tree or hive, or everything at once.
      if(!Number.isInteger(cmd.day))return {ok:false,message:'Harvest needs today’s date'};const r=ready(s,cmd.day);let gems=0,beds=0,picks=0;
      for(const p of r.beds){if(cmd.key&&cmd.key!==p.key)continue;gems+=bedYield(p);beds++;s.beds={...(s.beds||{}),[p.key]:cmd.day};}
      for(const o of r.daily){if(cmd.key&&cmd.key!==o.id)continue;gems+=catalog[o.type].daily;picks++;s.collected={...(s.collected||{}),[o.id]:cmd.day};}
      if(!beds&&!picks)return {ok:false,message:cmd.key?'Not ready yet. Come back tomorrow.':'Nothing is ready yet. Come back tomorrow.'};
      s.gems+=gems;message=`Harvest! +${gems} gems`+(beds?` from ${beds} garden bed${beds>1?'s':''}`:'')+(picks?`${beds?' and':' from'} ${picks} tree${picks>1?'s and hives':' or hive'}`:'')+'. It grows again by tomorrow.';
    }else if(cmd.kind==='seen-today'){if(!Number.isInteger(cmd.day))return {ok:false,message:'Missing day'};s.todaySeen=cmd.day;message='Have a lovely day at camp.';
    }else if(cmd.kind==='claim-request'){
      const f=cmd.family,key=`${cmd.day}:${f}`;if(!Number.isInteger(f)||f<0||f>12||typeof cmd.day!=='string'||cmd.day.length>12)return {ok:false,message:'Choose a guardian request'};
      if((s.requestsDone||[]).includes(key))return {ok:false,message:'Already delivered today. Thank you!'};
      if(!requestMet(s,f))return {ok:false,message:request(f).text};
      s.requestsDone=[...(s.requestsDone||[]),key].slice(-60);s.gems+=10;message='Delivered! +10 gems and a very happy guardian.';
    }else if(cmd.kind==='rename'){
      const name=String(cmd.name??'').replace(/\s+/g,' ').trim();if(!validName(name))return {ok:false,message:'Choose a name of up to 24 letters and spaces'};s.campName=name;message=`Welcome to ${name}!`;
      ...
      const c=s.challenge;if(!c)return {ok:false,message:'Start a review activity first'};const q=c.questions[c.index],correct=Number(cmd.answer)===(c.kind==='mill'?q.b:q.a*q.b);
      if(String(cmd.answer).trim()===''||!Number.isFinite(Number(cmd.answer)))return {ok:false,message:'Enter an answer'};
      ...
      else{c.index++;c.retry=false;c.feedback=false;message='That’s it!';if(c.index===c.questions.length){if(c.kind==='fish'){s.fish++;s.casts--;message='Fish caught! Trade it for gems or stone in town.';}else if(c.kind==='stone'){s.stone+=6;s.casts--;message='+6 river stones for your walls and cottage.';}else if(c.kind==='bake'){s.gems+=6;s.bakes=0;message='Fresh buns! Bram pays 6 gems.';}else if(c.kind==='mill'){s.wood+=5;s.mills=0;message='Fair shares! Millie gives you 5 planks of wood.';}else if(c.kind==='order'){s.gems+=5;s.wood+=2;s.orders=Math.max(0,(s.orders??2)-1);message='Order packed! Mara pays 5 gems and 2 wood.';}else if(c.kind==='tree'){s.cleared.push(c.target);s.wood+=6;message='+6 wood. This tree is cleared.';}else{const z=content.zones.find(z=>z.id===c.target);s.land.push(c.target);const t=forest.find(t=>content.inZone(z,Math.floor(t.x),Math.floor(t.z))&&treePresent(s,t));if(t)s.cleared.push(treeKey(t));s.wood+=6;message=z.name+' is now buildable! +6 wood. Keep or clear its remaining trees.';}s.challenge=null;}}
  ...
  /* ---------- building with math (0.41) ---------- */
  // Complete rectangles built from one kind of tile: seed plots make arrays, deck boards make areas.
  const PATTERN_TYPES=['plot','deck'];
  function patterns(save){const out=[];for(const type of PATTERN_TYPES){const cells=new Set(save.objects.filter(o=>o.type===type).map(o=>o.x+','+o.y)),seen=new Set();
      for(const k of cells){if(seen.has(k))continue;const stack=[k],comp=[];seen.add(k);while(stack.length){const c=stack.pop();comp.push(c);const [x,y]=c.split(',').map(Number);for(const [dx,dy] of [[1,0],[-1,0],[0,1],[0,-1]]){const n=(x+dx)+','+(y+dy);if(cells.has(n)&&!seen.has(n)){seen.add(n);stack.push(n);}}}
        const xs=comp.map(c=>+c.split(',')[0]),ys=comp.map(c=>+c.split(',')[1]),x=Math.min(...xs),y=Math.min(...ys),cols=Math.max(...xs)-x+1,rows=Math.max(...ys)-y+1;
        if(comp.length===rows*cols&&comp.length>=3)out.push({type,x,y,rows,cols,count:rows*cols,key:`${type}:${x},${y}:${rows}x${cols}`});}}
    return out;}
  const mathable=p=>p.rows>=2&&p.cols>=2&&p.rows<=12&&p.cols<=12;
  // Each guardian asks for something shaped by its own family.
  function request(f){return f===0?{family:0,text:'Could you build a deck for resting, at least 2 by 2?'}:f===1?{family:1,text:'Could you plant one long row of at least 3 seed plots?'}:{family:f,text:`Could you plant a garden bed with ${f} rows?`};}
  function requestMet(save,f){const ps=patterns(save);return f===0?ps.some(p=>p.type==='deck'&&p.rows>=2&&p.cols>=2):f===1?ps.some(p=>p.type==='plot'&&Math.min(p.rows,p.cols)===1&&p.count>=3):ps.some(p=>p.type==='plot'&&((p.rows===f&&p.cols>=2)||(p.cols===f&&p.rows>=2)));}
  /* ---------- come back tomorrow (0.44) ---------- */
  // A counted bed blooms the day it is counted and is ready to harvest from the next day on; trees and hives give once a day.
  const bedYield=p=>Math.max(2,Math.round(p.count/4));
  function ready(save,day){const counted=patterns(save).filter(p=>p.type==='plot'&&(save.solved||[]).includes(p.key));
    return {beds:counted.filter(p=>day-((save.beds||{})[p.key]??-1)>=1),daily:save.objects.filter(o=>catalog[o.type]?.daily&&((save.collected||{})[o.id]??-1)<day)};}
  function awardLearning(save,gems){if(!validSave(save))return save;const s=clone(migrateSave(save));s.gems+=Math.max(0,Math.floor(Number(gems)||0));s.harvested=[];s.casts=3;s.orders=2;s.bakes=1;s.mills=1;s.wood+=2;if(s.mastered>=3)s.stone+=4;s.revision++;return s;}

  function mount(host,options){
    const {getSave,setSave,profileId,avatar,onExit,onPractice}=options;
    let paused=true,recovery=null,loadTimer=0;
    let s=getSave();if(validSave(s)){s={...s,visits:(s.visits||0)+1};setSave(s);}
    let pattern=null,requestTold=false;const skippedPatterns=new Set();
    ...
    try{
    host.innerHTML=`<div class="cv2-shell"><canvas class="cv2-world" role="img" aria-label="Your living camp. Use the camp object list for accessible building."></canvas><div class="cv2-top"><header class="cv2-header"><button data-action="adventure" aria-label="Back to Adventure">‹</button><img class="cv2-profile-portrait" src="${esc(avatar)}" alt="Your selected explorer"><div class="cv2-brand"><small>YOUR WOODLAND HOME</small><h1>${esc(s.campName||'Willowbrook Camp')}</h1></div><button class="cv2-help" data-action="help" aria-label="Camp help">?</button><div class="cv2-purse" aria-label="Camp supplies"></div></header><div class="cv2-recovery-banner" role="status"></div></div><div class="cv2-play-space"><div class="cv2-camera"><button data-action="zoom-in" aria-label="Zoom in">+</button><button data-action="zoom-out" aria-label="Zoom out">−</button><button data-action="home" aria-label="Center camp">⌂</button><button data-action="orbit-left" aria-label="Rotate camera left">↶</button><button data-action="orbit-right" aria-label="Rotate camera right">↷</button></div><div class="cv2-world-notes"><div class="cv2-land-legend" aria-label="Land grid key"><span>Open land</span><span>Future land</span></div><div class="cv2-hint"></div><div class="cv2-announcement" role="status" aria-live="polite"></div></div><div class="cv2-overlay"></div></div><nav class="cv2-dock" aria-label="Camp tools">${[['explore','Explore'],['build','Build'],['bag','Backpack'],['journal','Journal']].map(([k,n])=>`<button data-action="${k}"><span aria-hidden="true">${ico[k]}</span>${n}</button>`).join('')}</nav></div>`;
    ...
    function recoveryMarkup(){return '<section class="cv2-card cv2-tray" aria-label="Camp help"><h2>'+ (recovery?'Let’s reopen your woodland':'Camp help')+'</h2><p>Restarting the view keeps your buildings, supplies and learning progress. Low power uses fewer graphics resources.</p><div class="cv2-recovery-actions"><button data-action="restart">Restart camp</button><button data-action="restart-low">Restart in low power</button><button data-action="reload">Reload app</button><button data-action="bag">Use Backpack controls</button><button data-action="adventure">Back to adventure</button><button data-action="close">Close help</button></div>'+ (recovery?'<details><summary>Technical details</summary><pre>'+esc(JSON.stringify(recovery,null,2))+'</pre></details>':'')+'</section>';}
    ...
    function act(cmd){explorer={x:Math.round(explorer.x),y:Math.round(explorer.y)};persistView();const before=clone(s),r=command(s,{...cmd,revision:s.revision},{tester:options.tester});if(!r.ok){tell(r.message);return false;}if(!commit(r.save))return false;if(cmd.kind==='finish-build'){if(undoStack.length)undoStack[undoStack.length-1].revision=s.revision;}else if(['place','move','upgrade','store','paint'].includes(cmd.kind)){undoStack.push({before,revision:s.revision});undoStack=undoStack.slice(-12);}else undoStack=[];undo=undoStack[undoStack.length-1]||null;preview=null;selected=null;walking=[];onArrive=null;seated=null;building=null;activity='';if(r.reviewEvent)options.onReview?.(r.reviewEvent);
      ...
      else if(cmd.kind==='answer-activity'&&r.reviewEvent?.correct&&!s.challenge)options.onSound?.('coin');
      tell(r.message);ui();return true;}
    function focusedPanel(){return !!panel;}
    function thumb(type){let picture;try{picture=!paused&&scene?.thumbnail(type);}catch(error){options.onDiagnostic?.({code:'thumbnail',detail:String(error.message||error).slice(0,300)});}if(picture)return '<img class="cv2-thumb model-preview" src="'+picture+'" alt="" aria-hidden="true">';const icons={tent:'⛺',canvas:'⛺',cabin:'⌂',fire:'♨',bench:'▰',lantern:'✧',path:'▱',deck:'▤',pine:'♠',feeder:'⌂',fence:'╫',gate:'⊓'};return '<span class="cv2-thumb modeled '+type+'" aria-hidden="true">'+(icons[type]||'⌂')+'</span>';}
    ...
    function ui(){
      ...
      // The land key only matters while building or viewing land; exploring keeps the world clear.
      host.classList.toggle('cv2-show-land',!!building||panel==='build'||panel==='land'||panel==='upgrades');
      const title=host.querySelector('.cv2-brand h1');if(title)title.textContent=s.campName||'Willowbrook Camp';
      const status=host.querySelector('.cv2-brand small');if(status){const h=homeStatus();status.textContent=h.text;status.title=h.text;}
      const sky=host.querySelector('.cv2-sky');if(sky){sky.textContent=s.night?'☀':'☾';sky.setAttribute('aria-pressed',String(!!s.night));sky.setAttribute('aria-label',s.night?'Switch to the morning sky':'Switch to the moonlit sky');}
      host.querySelector('.cv2-purse').innerHTML=`<strong>💎 ${s.gems}</strong><small>Wood ${s.wood} · Stone ${s.stone} · Fish ${s.fish}</small>`;
      host.classList.toggle('cv2-mode-build',mode==='build'||!!preview);
      host.querySelectorAll('.cv2-dock button').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.action===(panel||mode))));
      host.querySelector('.cv2-hint').textContent=preview?(preview.brush?'Build brush · tap squares to place · drag to pan':'Tap ground to preview. Confirm when it fits.'):(s.visits||0)>3?'':mode==='explore'?'Tap to walk · drag to pan · pinch to zoom':'Tap an object to edit, or choose something to build';
      let html='';
      if(panel==='help')html=recoveryMarkup();
      else if(s.challenge){const c=s.challenge,q=c.questions[c.index];html='<section class="cv2-card cv2-review" aria-label="Woodland review"><small>'+({fish:'FISHING',stone:'RIVER STONES',tree:'TREE CLEARING',land:'NEW LAND',order:'MARA’S ORDER',bake:'HONEYCRUST BAKERY',mill:'WILLOW WATERMILL'}[c.kind])+' · '+c.index+'/'+c.questions.length+' complete · No timer</small>'+(['order','bake','mill'].includes(c.kind)?jobPrompt(c.kind,q):'<h2>'+q.a+' × '+q.b+' = ?</h2>')+'<label for="camp-review-answer">Your answer</label><div class="cv2-review-entry"><input id="camp-review-answer" inputmode="numeric" autocomplete="off" maxlength="3" aria-label="Review answer"><button class="primary" data-action="answer-review">Check answer</button></div>'+ (c.feedback?'<p>Let’s see the groups. Your completed steps are safe.</p>'+root.MathVisuals.render(q.a,q.b):(c.kind==='order'?'<p>Mara pays 5 gems and 2 wood. Take the time you need.</p>':c.kind==='bake'?'<p>Bram pays 6 gems. Take the time you need.</p>':c.kind==='mill'?'<p>Millie gives 5 wood. Take the time you need.</p>':'<p>Two review facts earn useful supplies. Take the time you need.</p>'))+'<button data-action="leave-review">Leave activity</button></section>';}
      else if(panel==='land')html=guide.land(s,content);
      else if(panel==='upgrades')html=guide.upgrades(s,catalog,thumb,unlockReason,options.goalMarkup?.()||'');
      else if(panel==='welcome')html=guide.welcome(s);
      else if(!panel&&!preview&&!selected&&!pendingActivity&&(s.arrivals||[]).length)html=arrivalsMarkup();
      else if(panel==='pattern')html=patternMarkup()||(panel=null,'');
      else if(panel==='postcard')html=postcardMarkup();
      else if(panel==='travel')html=`<section class="cv2-card cv2-tray" aria-label="Where to?"><div class="cv2-tray-title"><div><small>THE CAMP SIGNPOST</small><h2>Where to?</h2></div><button data-action="close" aria-label="Close">×</button></div><div class="cv2-object-list">${content.locations.map(p=>`<button data-location="${p.id}">${esc(p.name)}<small>Hop straight there</small></button>`).join('')}</div></section>`;
        ...
        html+='</section>';
      }else if(panel==='place-info'){html=placeMarkup(selectedGuardian);
      }else if(panel==='goal-supplies'){
        html='<section class="cv2-card cv2-tray" aria-label="Supplies for my project"><div class="cv2-tray-title"><h2>One step closer to your project</h2><button data-action="close" aria-label="Close camp panel">×</button></div>'+ (options.goalMarkup?.()||'')+'<p>Learning adventures earn gems and refresh these fallen wood piles. The store trades earned gems or fish for supplies.</p><div class="cv2-object-list">'+sources.map((p,i)=>'<button data-source="'+i+'" '+(s.harvested.includes(i)?'disabled':'')+'>'+(s.harvested.includes(i)?'Already gathered':'Gather 3 wood')+'<small>Wood pile '+(i+1)+'</small></button>').join('')+'<button data-location="store">Visit the supply store<small>Wood and stone, bought with earned supplies</small></button><button data-action="practice">Earn supplies in a learning adventure</button></div></section>';
      }else if(panel==='river'){html='<section class="cv2-card cv2-tray" aria-label="River trip"><div class="cv2-tray-title"><div><small>WILLOWBROOK RIVER · '+s.casts+' TRIPS LEFT</small><h2>What will you gather?</h2></div><button data-action="close" aria-label="Close camp panel">×</button></div><p>Each trip is two review facts. A learning adventure brings three more trips.</p><div class="cv2-object-list"><button data-river="fish" '+(s.tools.includes('rod')&&s.casts>0?'':'disabled')+'>Go fishing<small>'+(s.tools.includes('rod')?'1 fish · trade for stone or gems':'Needs a fishing rod from the store')+'</small></button><button data-river="stone" '+(s.casts>0?'':'disabled')+'>Collect river stones<small>+4 stone</small></button></div></section>';}
      else if(panel==='store'){
        html='<section class="cv2-card cv2-tray" aria-label="Camp supply store"><div class="cv2-tray-title"><div><small>WILLOWBROOK VILLAGE</small><h2>Mara’s Supply Store</h2></div><button data-action="close" aria-label="Close camp panel">×</button></div>'+villagerSays('store')+'<p>Wood comes from your land and stone from the river. Trade fish here, or buy tools with gems.</p><div class="cv2-object-list"><button data-order="1" '+((s.orders??2)>0?'':'disabled')+'>Pack today’s order<small>'+((s.orders??2)>0?(s.orders??2)+' left · pays 5 gems + 2 wood':'More orders after your next learning adventure')+'</small></button>'+Object.entries(content.offers).map(([id,o])=>'<button data-buy="'+id+'" '+(o.tool&&s.tools.includes(o.tool)?'disabled':'')+'>'+o.name+'<small>'+(o.tool&&s.tools.includes(o.tool)?'Owned':o.fishCost?'1 fish':o.price+' gems'+(o.woodCost?' + '+o.woodCost+' wood':'')+(o.need?' · '+o.need+' families':''))+'</small></button>').join('')+'</div></section>';
        ...
        // Land and home upgrades live with building and planning, keeping the first view clear.
        if(panel==='build'||panel==='journal')html+='<div class="cv2-panel-links"><button data-action="land">My land</button><button data-action="upgrades">Home upgrades</button>'+(panel==='journal'?'<button data-action="help">Camp help</button>':'')+'</div>';
        if(panel==='build'||panel==='bag'){
          ...
          html+=`<h3>Fallen wood</h3><div class="cv2-object-list">${sources.map((p,i)=>`<button data-source="${i}" ${s.harvested.includes(i)?'disabled':''}>${s.harvested.includes(i)?'Gathered':'Gather 3 wood'}<small>Wood pile ${i+1}</small></button>`).join('')}</div><div class="cv2-note"><strong>A place to return to, at your pace.</strong><p>Your starting kit is yours to keep. Open Upgrades to see how your home can grow. A finished learning round earns camp gems and brings more fallen wood. Your earlier camp collection is preserved in Parents. Fences are for creative play; nothing attacks or damages this clearing.</p></div><button class="primary" data-action="practice">A short learning adventure →</button><div class="cv2-settings"><button data-action="calm" aria-pressed="${!!s.calm}">Gentle motion: ${s.calm?'on':'off'}</button><button data-action="quality" aria-pressed="${s.quality==='low'}">Low power: ${s.quality==='low'?'on':'off'}</button><button data-action="night" aria-pressed="${!!s.night}">${s.night?'Moonlit':'Morning'} scenery</button>${options.onToggleSound?`<button data-action="sounds" aria-pressed="${!!options.soundOn?.()}">Camp sounds: ${options.soundOn?.()?'on':'off'}</button>`:''}</div><h3>Make it yours</h3><div class="cv2-name"><label for="camp-name">Name your camp</label><div class="cv2-review-entry"><input id="camp-name" maxlength="24" autocomplete="off" value="${esc(s.campName||'')}" placeholder="Willowbrook Camp"><button data-action="rename">Save name</button></div></div><button class="primary" data-action="postcard">Take a postcard of my camp</button>`;
        }html+='</section>';
      }else if(pendingActivity){const p=pendingActivity,z=p.kind==='land'?content.zones.find(z=>z.id===p.target):null;html=`<section class="cv2-card cv2-offer" aria-label="${p.kind==='land'?'Open new land':'Clear a tree'}"><small>${p.kind==='land'?'NEW LAND':'TREE CLEARING'} · NO TIMER</small><h2>${p.kind==='land'?'Open '+esc(z?.name||'this land')+'?':'Clear this tree?'}</h2><p>${p.kind==='land'?(s.mastered<(z?.need||0)?'Restore '+z.need+' realms to open this land.':'Two review facts open it for building, and bring 6 wood.'):'Two review facts clear it and bring 6 wood. Trees you keep stay part of your world.'}</p><div class="cv2-actions"><button data-action="decline-activity">Not now</button>${p.kind==='land'&&s.mastered<(z?.need||0)?'':`<button class="primary" data-action="confirm-activity">${p.kind==='land'?'Open it':'Clear it'}</button>`}</div></section>`;
      }else if(selected){const o=s.objects.find(o=>o.id===selected);if(o){const d=catalog[o.type];html=`<section class="cv2-card cv2-selection" aria-label="Selected ${d.name}"><div class="cv2-card-heading">${thumb(o.type)}<div><small>YOUR CAMP</small><h2>${d.name}</h2><p>${placeName(o.x,o.y)}</p></div><button data-action="close" aria-label="Close selection">×</button></div><div class="cv2-actions">${d.action?`<button class="primary" data-action="use">${d.action}</button>`:''}<button data-action="move">Move</button>${d.connect?'<button data-action="build-more">Build more</button>':''}<button data-action="store">Store</button><div class="cv2-swatches" role="group" aria-label="Choose a colour">${SWATCHES.map((c,i)=>`<button data-paint="${i}" style="--swatch:${c[0]}" aria-label="${c[1]}" aria-pressed="${(o.color||0)===i}"></button>`).join('')}</div>${d.benefit?`<p>${d.benefit}</p>`:''}${d.next?`<button data-action="upgrade" ${unlockReason(s,d.next)&&!(s.inventory[d.next]>0)?'disabled':''}>${s.inventory[d.next]>0?'Use renovation kit · free':unlockReason(s,d.next)?'Unlock at '+catalog[d.next].need+' families':'Upgrade · 💎 '+catalog[d.next].price+' / '+catalog[d.next].wood+' wood'+(catalog[d.next].stone?' / '+catalog[d.next].stone+' stone':'')}</button>`:''}</div></section>`;}}
      if(!html)html=`<div class="cv2-idle"><span>${activity||'Your next adventure starts here.'}</span><button data-action="undo" ${undo&&undo.revision===s.revision?'':'disabled'} aria-label="Undo last camp action">↶ Undo</button></div>`;
    ...
    /* Children see places, not grid coordinates. */
    function placeName(x,y){if(x>=0&&x<COLS&&y>=0&&y<ROWS)return 'In your clearing';const z=content.zones.find(z=>content.inZone(z,x,y));return z?z.name:'In the woods';}
    /* Decorations in the way of a growing home can be set aside for free (each step can be undone). Homes are never moved automatically. */
    function roomMakers(p){if(!p)return [];const size=footprint(p.type,p.r),box={x:p.x,y:p.y,...size},layer=catalog[p.type].layer;const hits=s.objects.filter(o=>o.id!==p.id&&catalog[o.type].layer===layer&&overlaps(box,{...o,...footprint(o.type,o.r)}));return hits.length&&hits.every(o=>!HOMES.includes(o.type)&&s.construction?.objectId!==o.id)?hits:[];}
    /* Show a whole building: zoom out enough for its footprint, then centre it. */
    function frameObject(o){const f=footprint(o.type,o.r),size=Math.max(f.w,f.h),fit=size>=4?.95:size===3?1.1:1.35;if(camera.zoom>fit)camera=reframeCamera(camera,{zoom:fit});scene?.focus(o.x+f.w/2,o.y+f.h/2,camera,true);dirty=true;}
    /* One line in the header: your home now, and exactly what the next renovation needs. */
    /* News from the adventure: kits that arrived in the backpack and keepsakes that found a place by the Story Stones. */
    function arrivalsMarkup(){const list=s.arrivals||[],kits=list.filter(a=>a.kind==='kit'),keeps=list.filter(a=>a.kind==='keepsake'),placed=keeps.filter(a=>Number.isInteger(a.x));
      const row=a=>`<li>${thumb(a.type)}<span><strong>${esc(catalog[a.type].name)}${a.count>1?' × '+a.count:''}</strong><small>${a.kind==='kit'?'Ready to place, free':Number.isInteger(a.x)?'Now standing by the Story Stones':'Waiting in your backpack'}</small></span></li>`;
      return `<section class="cv2-card cv2-arrivals" aria-label="New at camp"><div class="cv2-tray-title"><small>FROM YOUR ADVENTURES</small><button data-action="arrivals-ok" aria-label="Close">×</button></div><h2>${kits.length?'New things for your camp!':'A keepsake came home'}</h2><p>${kits.length?'Every realm you restore sends something to build. Kits are free to place.':'Restored realms send a keepsake to the Story Stones.'}</p><ul class="cv2-arrival-list">${list.slice(-12).map(row).join('')}</ul><div class="cv2-actions">${placed.length?'<button data-action="arrivals-see">See the Story Stones</button>':''}${kits.length?'<button data-action="arrivals-bag" class="primary">Open my backpack</button>':'<button data-action="arrivals-ok" class="primary">Lovely!</button>'}</div></section>`;}
    /* Building with math: a finished rectangle of seed plots or deck boards asks to be counted. */
    function checkPatterns(){if(s.challenge||panel)return;const p=patterns(s).find(p=>mathable(p)&&!(s.solved||[]).includes(p.key)&&!skippedPatterns.has(p.key));if(p){pattern=p.key;panel='pattern';}
      const r=options.request;if(!requestTold&&r&&requestMet(s,r.family)&&!(s.requestsDone||[]).includes(r.day+':'+r.family)){requestTold=true;tell(r.name+' would love that! Open Journal to show them.');}}
    function patternMarkup(){const p=patterns(s).find(p=>p.key===pattern);if(!p)return '';const plot=p.type==='plot',tried=s.patternTry===p.key,noun=plot?'seed plots':'deck squares';
      const grid=`<div class="cv2-array ${p.type}" aria-hidden="true">${Array.from({length:p.rows},(_,r)=>`<div class="cv2-array-row">${'<i></i>'.repeat(p.cols)}${tried?`<b>${r===p.rows-1?'?':(r+1)*p.cols}</b>`:''}</div>`).join('')}</div>`;
      return `<section class="cv2-card cv2-pattern" aria-label="Count what you built"><small>${plot?'YOUR GARDEN IS AN ARRAY':'YOUR DECK HAS AN AREA'}</small><h2>${p.rows} rows of ${p.cols}</h2>${grid}<p>${tried?'Skip-count down the rows: '+Array.from({length:p.rows-1},(_,r)=>(r+1)*p.cols).join(', ')+', … what comes next?':`How many ${noun} did you build?`}</p><div class="cv2-review-entry"><input id="camp-pattern-answer" inputmode="numeric" autocomplete="off" maxlength="3" aria-label="How many ${noun}"><button class="primary" data-action="solve-pattern">Check</button></div><button data-action="skip-pattern">Not now</button></section>`;}
    function orderPrompt(q){const crates=Array.from({length:q.a},()=>`<span class="cv2-crate">${'<i></i>'.repeat(q.b)}</span>`).join('');
      return `<h2>${q.a} crates of ${q.b}</h2><p>Mara needs ${q.a} crate${q.a===1?'':'s'} with ${q.b} apple${q.b===1?'':'s'} in each. How many apples is that?</p>${q.a>0&&q.b>0&&q.a*q.b<=144?`<div class="cv2-crates" aria-hidden="true">${crates}</div>`:''}`;}
    function requestMarkup(){const r=options.request;if(!r||!Number.isInteger(r.family))return '';const q=request(r.family),done=(s.requestsDone||[]).includes(r.day+':'+r.family),met=requestMet(s,r.family);
      return `<section class="cv2-request"><small>TODAY’S GUARDIAN REQUEST</small><h3>${esc(r.name)} asks:</h3><p>“${esc(q.text)}”</p>${done?'<p class="cv2-done">✓ Delivered. Thank you!</p>':met?'<button class="primary" data-action="claim-request">Show '+esc(r.name)+' · +10 gems</button>':'<p class="cv2-sub">Build it anywhere on your land. Seed plots and decks are in Build.</p>'}</section>`;}
    /* A postcard of the camp: the current view, framed on paper with the camp's name and the date. */
    let postcard=null;
    function makePostcard(){const shot=scene?.snapshot?.();if(!shot)return Promise.resolve(null);return new Promise(resolve=>{const img=new Image();img.onload=()=>{const W=1200,H=820,c=document.createElement('canvas');c.width=W;c.height=H;const g=c.getContext('2d');
      g.fillStyle='#f1e4c6';g.fillRect(0,0,W,H);for(let i=0;i<2600;i++){g.fillStyle=`rgba(${Math.random()<.5?'120,90,50':'255,250,235'},${.04+Math.random()*.08})`;g.fillRect(Math.random()*W,Math.random()*H,1.5,1.5);}
      const bx=48,by=48,bw=W-96,bh=560,r=Math.max(bw/img.width,bh/img.height),sw=bw/r,sh=bh/r;g.fillStyle='#fffaf0';g.fillRect(bx-10,by-10,bw+20,bh+20);g.drawImage(img,(img.width-sw)/2,(img.height-sh)/2,sw,sh,bx,by,bw,bh);g.strokeStyle='#3b2a1c';g.lineWidth=2;g.strokeRect(bx,by,bw,bh);
      g.fillStyle='#3b2a1c';g.font='italic 52px Georgia, serif';g.fillText('Greetings from '+(s.campName||'Willowbrook Camp')+'!',bx,by+bh+86,bw-220);
      g.font='26px Georgia, serif';g.fillStyle='#5a4430';g.fillText(new Date().toLocaleDateString(undefined,{day:'numeric',month:'long',year:'numeric'})+' · '+(options.season||'summer')+' · '+(options.ownerName?options.ownerName+'’s camp':'my camp'),bx,by+bh+132,bw-220);
      const sx=W-190,sy=by+bh+38;g.fillStyle='#fffaf0';g.fillRect(sx,sy,130,150);g.setLineDash([6,5]);g.strokeStyle='#8b6a44';g.strokeRect(sx+6,sy+6,118,138);g.setLineDash([]);g.fillStyle='#2f6a4a';g.beginPath();g.moveTo(sx+65,sy+26);g.lineTo(sx+100,sy+104);g.lineTo(sx+30,sy+104);g.closePath();g.fill();g.fillStyle='#6b4a2e';g.fillRect(sx+59,sy+104,12,18);g.fillStyle='#3b2a1c';g.font='bold 18px Georgia, serif';g.fillText('×12',sx+44,sy+140);
      const t=document.createElement('canvas');t.width=360;t.height=246;t.getContext('2d').drawImage(c,0,0,360,246);try{options.savePostcard?.(t.toDataURL('image/jpeg',.7));}catch{}
      resolve(c.toDataURL('image/jpeg',.88));};img.onerror=()=>resolve(null);img.src=shot;});}
    function postcardMarkup(){if(!postcard)return '<section class="cv2-card cv2-tray" aria-label="Postcard"><h2>Developing your postcard…</h2></section>';
      return `<section class="cv2-card cv2-tray cv2-postcard" aria-label="Your camp postcard"><div class="cv2-tray-title"><div><small>A POSTCARD FROM CAMP</small><h2>Greetings from ${esc(s.campName||'Willowbrook Camp')}!</h2></div><button data-action="close" aria-label="Close postcard">×</button></div><img src="${postcard}" alt="A postcard picture of your camp"><div class="cv2-actions">${navigator.canShare?'<button class="primary" data-action="share-postcard">Share with a grown-up</button>':''}<a class="cv2-button" download="${esc((s.campName||'Willowbrook Camp').replace(/[^\w ]/g,''))} postcard.jpg" href="${postcard}">Save picture</a></div></section>`;}
    /* The village (0.43): each place has its person, a line for today, and a job. */
    function villagerSays(id){const v=(content.villagers||[]).find(v=>v.place===id);if(!v)return '';const line=v.lines[((options.today||0)+v.id.length)%v.lines.length];return `<p class="cv2-says"><strong>${esc(v.name)} the ${esc(v.role)}:</strong> “${esc(line)}”</p>`;}
    function libraryMarkup(){const facts=new Set((s.factsBuilt||[]).map(f=>f.split(':')[1]));const cells=[];for(let a=2;a<=12;a++)for(let b=a;b<=12;b++)cells.push(`<span class="${facts.has(a+'x'+b)?'got':''}">${a}×${b}</span>`);const cards=options.postcards?.()||[];
      return `<h3>Your fact garden album · ${facts.size}/66</h3><p>Every garden bed or deck you count is written in here.</p><div class="cv2-album" aria-label="${facts.size} of 66 facts built">${cells.join('')}</div><h3>Postcards on the wall</h3>${cards.length?`<div class="cv2-postcards">${cards.map(u=>`<img src="${u}" alt="A postcard of your camp">`).join('')}</div>`:'<p>Take a postcard from the Journal and Wren will pin it up here.</p>'}`;}
    function placeMarkup(id){const place=content.locations.find(p=>p.id===id);let body='';
      if(id==='bakery')body=`<p>Warm bread, honey buns, and one baking order each learning adventure.</p><div class="cv2-object-list"><button data-job="bake" ${(s.bakes??1)>0?'':'disabled'}>Bake today’s trays<small>${(s.bakes??1)>0?'Bram pays 6 gems':'Bram bakes again after your next learning adventure'}</small></button></div>`;
      else if(id==='mill')body=`<p>The wheel saws logs into planks, shared out fairly.</p><div class="cv2-object-list"><button data-job="mill" ${(s.mills??1)>0?'':'disabled'}>Share out the logs<small>${(s.mills??1)>0?'Millie gives 5 wood':'The wheel turns again after your next learning adventure'}</small></button></div>`;
      else if(id==='library')body=libraryMarkup();
      else if(id==='inn')body=`<h3>The notice board</h3>${requestMarkup()||'<p>No guardian requests yet. Restore a realm and its guardian will write to you here.</p>'}<ul class="cv2-notices">${options.visitorName?`<li>${esc(options.visitorName)} is visiting the Story Stones today.</li>`:''}<li>Mara has ${s.orders??2} order${(s.orders??2)===1?'':'s'} to pack.</li><li>Bram ${(s.bakes??1)>0?'has a baking order':'has finished baking'}; Millie ${(s.mills??1)>0?'has logs to share':'is resting'}.</li></ul>`;
      else if(id==='workshop')body=`<p>Tink can show you the next shape of every home. To repaint something, tap it in your camp and choose a colour.</p><button class="primary" data-action="upgrades">See home upgrades</button>`;
      else body=`<p>${({market:'A market of striped awnings, handmade supplies and neighbours.',gardens:'A quiet garden for every explorer. Follow the rose walk and find the fountain.'})[id]||'There is more to discover here.'}</p><button data-location="store">Visit Mara’s store</button>`;
      return `<section class="cv2-card cv2-tray" aria-label="${esc(place?.name||'Willowbrook')}"><div class="cv2-tray-title"><div><small>WILLOWBROOK VILLAGE</small><h2>${esc(place?.name||'Willowbrook')}</h2></div><button data-action="close" aria-label="Close">×</button></div>${villagerSays(id)}${body}</section>`;}
    function jobPrompt(kind,q){if(kind==='order')return orderPrompt(q);
      if(kind==='bake')return `<h2>${q.a} trays of ${q.b}</h2><p>Bram needs ${q.a} tray${q.a===1?'':'s'} with ${q.b} bun${q.b===1?'':'s'} on each. How many buns is that?</p>${q.a*q.b<=144&&q.a>0&&q.b>0?`<div class="cv2-crates" aria-hidden="true">${Array.from({length:q.a},()=>`<span class="cv2-crate cv2-tray-buns">${'<i></i>'.repeat(q.b)}</span>`).join('')}</div>`:''}`;
      return `<h2>${q.a*q.b} logs, ${q.a} stacks</h2><p>Millie has ${q.a*q.b} logs to share into ${q.a} equal stack${q.a===1?'':'s'}. How many logs go in each stack?</p>`;}
    function homeStatus(){const order=HOMES;let best=null;for(const o of s.objects)if(order.includes(o.type)&&(!best||order.indexOf(o.type)>order.indexOf(best.type)))best=o;if(!best)return {text:'YOUR WOODLAND HOME'};const d=catalog[best.type],next=d.next&&catalog[d.next];if(!next)return {text:'YOUR '+d.name.toUpperCase()+' · FULLY GROWN'};const kit=(s.inventory[d.next]||0)>0,needs=[];const realms=Math.max(0,(next.need||0)-s.mastered);if(realms)needs.push(realms+' realm'+(realms===1?'':'s'));if(!kit){if(s.gems<next.price)needs.push((next.price-s.gems)+' gems');if(s.wood<(next.wood||0))needs.push((next.wood-s.wood)+' wood');if(s.stone<(next.stone||0))needs.push((next.stone-s.stone)+' stone');}return {text:'YOUR '+d.name.toUpperCase()+' · NEXT: '+next.name.toUpperCase()+(needs.length?' · NEEDS '+needs.join(', ').toUpperCase():' · READY')};}
    ...
    function finishAndMoveIn(){const id=s.construction?.objectId;act({kind:'finish-build'});const home=s.objects.find(o=>o.id===id&&HOMES.includes(o.type));if(home&&!s.construction){movingIn=home.id;useObject(home);}}
    function useObject(o){const targets=catalog[o.type].layer==='surface'?[{x:o.x,y:o.y}]:entrances(o);const to=targets.find(p=>route(s,explorer,p)!==null);if(!to){tell('Leave an open route beside this object.');return;}const d=catalog[o.type];travel(to,()=>{scene?.focus(o.x+d.w/2,o.y+d.h/2,camera,true);if(['bench','tent','trailtent','canvas','cabin','lodge','stonehome','keep'].includes(o.type))seated=o;activity=HOMES.includes(o.type)?(o.id===movingIn?'Moving-in day! Your buddy explores every corner of your new '+catalog[o.type].name.toLowerCase()+'.':'Your buddy settles beside you. A little home, made by you.'):o.type==='bench'?'A good seat. A very good dog.':o.type==='feeder'?'A small bird stops by to say hello.':o.type==='fire'?'Warm paws, warm stories.':'A quiet moment at home.';tell(activity);},`Walking to ${d.name.toLowerCase()}`);}
    function dispatch(e){const b=e.target.closest('button');if(!b||b.disabled)return;
      ...
      if(b.dataset.order){panel=null;startWoodland('order');return;}
      if(b.dataset.job){panel=null;startWoodland(b.dataset.job);return;}
      if(b.dataset.location){visitLocation(b.dataset.location,true);return;}
      if(b.dataset.river){panel=null;startWoodland(b.dataset.river==='stone'?'stone':'fish');return;}
      ...
      if(b.dataset.select){selected=b.dataset.select;panel=null;const item=s.objects.find(o=>o.id===selected);if(item)frameObject(item);ui();host.querySelector('[data-action="move"]')?.focus();return;}
      const a=b.dataset.action,o=s.objects.find(o=>o.id===selected);
      if(a==='make-room'&&preview){const keep=preview,movers=roomMakers(preview);for(const m of movers)act({kind:'store',id:m.id});preview=keep;tell(movers.length+' decoration'+(movers.length===1?'':'s')+' moved to your Backpack. Undo brings them back.');ui();return;}
      if(a==='confirm-activity'&&pendingActivity){const p=pendingActivity;pendingActivity=null;startWoodland(p.kind,p.target);return;}
      if(a==='decline-activity'){pendingActivity=null;ui();return;}
      if(a==='help'){panel='help';ui();return;}
      if(a==='dismiss-slow'){const el=host.querySelector('.cv2-recovery-banner');if(el)el.innerHTML='';return;}
      if(a==='restart'||a==='restart-low'){persistView();options.onRestart?.(a==='restart-low');return;}
      if(a==='reload'){persistView();options.onReload?.();return;}
      if(a==='adventure'){persistView();onExit('screen-map');return;}
      ...
      if(a==='finish-build'){finishAndMoveIn();return;}
      if(a==='paint'&&o){act({kind:'paint',id:o.id});return;}
      if(b.dataset.paint!==undefined&&o){const id=o.id;act({kind:'paint',id,color:Number(b.dataset.paint)});selected=id;ui();return;}
      ...
      if(a==='solve-pattern'&&pattern){const key=pattern,v=host.querySelector('#camp-pattern-answer')?.value??'';if(act({kind:'solve-pattern',key,answer:v})){if((s.solved||[]).includes(key)){pattern=null;panel=null;checkPatterns();}else panel='pattern';}ui();host.querySelector('#camp-pattern-answer')?.focus();return;}
      if(a==='skip-pattern'){if(pattern)skippedPatterns.add(pattern);pattern=null;panel=null;ui();return;}
      if(a==='claim-request'&&options.request){act({kind:'claim-request',family:options.request.family,day:options.request.day});panel='journal';ui();return;}
      if(a==='rename'){const v=host.querySelector('#camp-name')?.value??'';if(act({kind:'rename',name:v}))panel='journal';ui();return;}
      if(a==='sounds'){options.onToggleSound?.();ui();return;}
      if(a==='postcard'){panel='postcard';postcard=null;ui();makePostcard().then(url=>{if(disposed)return;postcard=url;if(!url){panel=null;tell('The camera needs the 3D view. Try again when your camp is showing.');}ui();});return;}
      if(a==='share-postcard'&&postcard){fetch(postcard).then(r=>r.blob()).then(b=>{const file=new File([b],'camp-postcard.jpg',{type:'image/jpeg'});return navigator.canShare?.({files:[file]})?navigator.share({files:[file],title:s.campName||'Willowbrook Camp'}):null;}).catch(()=>{});return;}
      if(a==='arrivals-ok'||a==='arrivals-bag'||a==='arrivals-see'){commit({...s,arrivals:[]});panel=a==='arrivals-bag'?'bag':null;if(a==='arrivals-see'){camera={...camera,zoom:Math.max(1.3,camera.zoom)};scene?.focus(-5,2,camera,true);persistView();}dirty=true;ui();return;}
      ...
      if(a==='undo'){if(undo&&undo.revision===s.revision){const previous=undo.before;previous.revision=s.revision+1;previous.camera={...camera};walking=[];onArrive=null;seated=null;building=null;activity='';explorer={...previous.explorer};pet={x:explorer.x+.5,y:explorer.y+.3};commit(previous);undoStack.pop();if(undoStack.length)undoStack[undoStack.length-1].revision=s.revision;undo=undoStack[undoStack.length-1]||null;if(preview)preview.lastPlaced=false;tell(undo?`Undone. ${undoStack.length} more step${undoStack.length===1?'':'s'} can be undone.`:'Last action undone, including supplies and project rewards.');}ui();return;}
      if(a==='close'||a==='cancel'){if(panel==='welcome')commit({...s,openingIntro:false});const brushed=!!preview?.brush;if(brushed)mode='explore';preview=null;selected=null;panel=null;if(brushed)checkPatterns();ui();host.querySelector(`[data-action="${mode}"]`)?.focus();return;}
      if(['explore','build','bag','journal','land','upgrades'].includes(a)){if((s.arrivals||[]).length)commit({...s,arrivals:[]});preview=null;selected=null;if(a==='explore'){mode='explore';panel=null;}else{mode=a==='build'?'build':mode;panel=panel===a?null:a;}ui();return;}
    ...
    function placePreview(){const p={...preview};if(act({kind:p.upgrade?'upgrade':p.id?'move':'place',...p})){if(p.brush)preview={...p,lastPlaced:true};else checkPatterns();}ui();return;}
    function tap(px,py){if(paused)return;if(s.challenge)return;if(walking.length){skipWalk();return;}if(pendingActivity){pendingActivity=null;ui();return;}if(focusedPanel()){panel=null;ui();return;}const p=cell(px,py);if(preview){preview.x=p.x;preview.y=p.y;preview.lastPlaced=false;if(preview.brush)placePreview();else ui();return;}
      const hit=scene?.pick(px,py);
      if(hit?.guardian!==undefined){visitGuardian(hit.guardian);return;}
      if(hit?.location==='signpost'){panel='travel';selected=null;ui();return;}
      if(hit?.location){visitLocation(hit.location);return;}
      // Reviews never start from a stray tap: the child confirms first.
      if(hit?.tree){offerActivity('tree',hit.tree);return;}
      if(hit?.land){if(s.land.includes(hit.land))startWoodland('land',hit.land);else offerActivity('land',hit.land);return;}
    ...
    function resizeUnsafe(){const oldScale=width&&height?Math.min(height/19,width/18):0;width=host.clientWidth||root.innerWidth;height=host.clientHeight||root.innerHeight;if(oldScale){const ratio=Math.min(height/19,width/18)/oldScale;camera.x*=ratio;camera.y*=ratio;}scene?.resize(width,height,s.quality);dirty=true;}
    function resize(){try{resizeUnsafe();}catch(error){fail('resize',error);}}
    /* Frame pacing is measured on the child's own device. A slow camp is offered low power once; nothing changes without a tap. */
    const frameTimes=[];let lastFrame=0,slowOffered=false;
    function samplePace(t){
      const d=lastFrame?t-lastFrame:0;lastFrame=t;if(d<=0||d>1000)return;
      frameTimes.push(d);if(frameTimes.length>240)frameTimes.shift();
      if(frameTimes.length%60===0){const sorted=[...frameTimes].sort((a,b)=>a-b),median=sorted[Math.floor(sorted.length/2)],p95=sorted[Math.floor(sorted.length*.95)];
        root.CampPerf={median:Math.round(median*10)/10,p95:Math.round(p95*10)/10,samples:frameTimes.length,quality:s.quality||'standard',at:new Date().toISOString()};
        if(!slowOffered&&frameTimes.length>=180&&median>40&&s.quality!=='low'){slowOffered=true;const el=host.querySelector('.cv2-recovery-banner');if(el&&!recovery)el.innerHTML='<span>Willowbrook is running slowly on this device. Low power keeps it smooth and keeps everything you built.</span><button data-action="restart-low">Use low power</button><button data-action="dismiss-slow">Not now</button>';}}
    }
    function frame(t){if(disposed||paused){raf=0;lastFrame=0;return;}raf=requestAnimationFrame(frame);samplePace(t);try{frameWorld(t);}catch(error){fail('render',error);}}
    function frameWorld(t){if(document.hidden){last=t;return;}const gap=s.quality==='low'?33:16;if(t-last<gap)return;const dt=Math.min(.05,(t-last)/1000||.016);last=t;
      if(s.construction&&!s.challenge&&!panel)resumeBuild();if(building&&performance.now()-building.started>3500)finishAndMoveIn();
    ...
    loadTimer=setTimeout(()=>fail('loading-timeout','No world loaded within 15 seconds. Try again or reload while online.'),15000);
    (options.loadScene?options.loadScene():import('./camp-v2-scene.js')).then(module=>{
      if(disposed||recovery)return;
      scene=module.createScene(canvas,{patterns,companions:((options.companions||[]).length?options.companions:options.visitors||[]).map(c=>({art:c.art,name:c.name})),season:options.season||'summer',catalog,footprint,sources,world,avatar,guardians,placementReason,ownerName:options.ownerName||'',visitor:options.visitor??null,goldRealms:options.goldRealms||[],lowPower:s.quality==='low',
        onContextRestored:()=>{if(disposed||!scene||!paused)return;paused=false;recovery=null;raf=requestAnimationFrame(frame);host.classList.remove('cv2-no-webgl');canvas.setAttribute('aria-label','3D woodland camp. Tap to walk, drag to pan. Backpack offers accessible object controls.');dirty=true;recoveryBanner();ui();tell('Your woodland is ready again.');},
        onContextLost:()=>fail('context-lost','The browser released its WebGL graphics context. This can happen after backgrounding or under memory pressure.')});
