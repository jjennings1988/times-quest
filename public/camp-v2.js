...
(function (root) {
  ...
  function validSave(s){
    ...
    if(s.v===3){
      ...
      if(s.challenge&&(!['fish','stone','tree','land'].includes(s.challenge.kind)||!Array.isArray(s.challenge.questions)||![2,3].includes(s.challenge.questions.length)||!Number.isInteger(s.challenge.index)||s.challenge.index<0||s.challenge.index>=s.challenge.questions.length||s.challenge.questions.some(q=>!Number.isInteger(q.a)||!Number.isInteger(q.b)||q.a<0||q.a>12||q.b<0||q.b>12)))return false;
    }
  ...
  function command(save,cmd,context={}){
    ...
    if(['place','move','upgrade'].includes(cmd.kind)){
      ...
      if(!Array.isArray(cmd.questions)||![2,3].includes(cmd.questions.length)||cmd.questions.some(q=>!Number.isInteger(q.a)||!Number.isInteger(q.b)||q.a<0||q.a>12||q.b<0||q.b>12))return {ok:false,message:'Review questions are unavailable'};
      s.challenge={kind,target:target||null,questions:cmd.questions.map(q=>({a:q.a,b:q.b})),index:0,retry:false,feedback:false};message=(cmd.questions.length===2?'Two':'Three')+' review facts, at your pace. Mistakes are for learning.';
  ...
  function awardLearning(save,gems){if(!validSave(save))return save;const s=clone(migrateSave(save));s.gems+=Math.max(0,Math.floor(Number(gems)||0));s.harvested=[];s.casts=3;s.wood+=2;if(s.mastered>=3)s.stone+=2;s.revision++;return s;}

  function mount(host,options){
    const {getSave,setSave,profileId,avatar,onExit,onPractice}=options;
    let paused=true,recovery=null,loadTimer=0;
    let s=getSave(),disposed=false,mode='explore',panel=s.openingIntro?'welcome':null,selected=null,preview=null,undo=null,undoStack=[],pendingActivity=null;
    ...
    try{
    host.innerHTML=`<div class="cv2-shell"><canvas class="cv2-world" role="img" aria-label="Your living camp. Use the camp object list for accessible building."></canvas><div class="cv2-top"><header class="cv2-header"><button data-action="adventure" aria-label="Back to Adventure">‹</button><img class="cv2-profile-portrait" src="${esc(avatar)}" alt="Your selected explorer"><div class="cv2-brand"><small>YOUR WOODLAND HOME</small><h1>Willowbrook Camp</h1></div><button class="cv2-help" data-action="help" aria-label="Camp help">?</button><div class="cv2-purse" aria-label="Camp supplies"></div></header><div class="cv2-recovery-banner" role="status"></div></div><div class="cv2-play-space"><div class="cv2-camera"><button data-action="zoom-in" aria-label="Zoom in">+</button><button data-action="zoom-out" aria-label="Zoom out">−</button><button data-action="home" aria-label="Center camp">⌂</button><button data-action="orbit-left" aria-label="Rotate camera left">↶</button><button data-action="orbit-right" aria-label="Rotate camera right">↷</button><button data-action="night" class="cv2-sky" aria-label="Change the sky">☾</button></div><div class="cv2-world-notes"><div class="cv2-land-legend" aria-label="Land grid key"><span>Open land</span><span>Future land</span></div><div class="cv2-hint"></div><div class="cv2-announcement" role="status" aria-live="polite"></div></div><div class="cv2-overlay"></div></div><nav class="cv2-dock" aria-label="Camp tools">${[['explore','Explore'],['build','Build'],['bag','Backpack'],['journal','Journal']].map(([k,n])=>`<button data-action="${k}"><span aria-hidden="true">${ico[k]}</span>${n}</button>`).join('')}</nav></div>`;
    ...
    (options.loadScene?options.loadScene():import('./camp-v2-scene.js')).then(module=>{
      ...
      scene=module.createScene(canvas,{catalog,footprint,sources,world,avatar,guardians,placementReason,ownerName:options.ownerName||'',visitor:options.visitor??null,goldRealms:options.goldRealms||[],lowPower:s.quality==='low',
