...
(function (root) {
  ...
  function mount(host,options){
    ...
    function ui(){
      ...
      if(panel==='help')html=recoveryMarkup();
        ...
        if(panel==='build'||panel==='bag'){
          ...
          html+=`<h3>Fallen wood</h3><div class="cv2-object-list">${sources.map((p,i)=>`<button data-source="${i}" ${s.harvested.includes(i)?'disabled':''}>${s.harvested.includes(i)?'Gathered':'Gather 3 wood'}<small>Wood pile ${i+1} · column ${p.x+1}, row ${p.y+1}</small></button>`).join('')}</div><div class="cv2-note"><strong>A place to return to, at your pace.</strong><p>Your starting kit is yours to keep. Open Upgrades to see how your home can grow. A finished learning round earns camp gems and brings more fallen wood. Your earlier camp collection is preserved in Parents. Fences are for creative play; nothing attacks or damages this clearing.</p></div><button class="primary" data-action="practice">A short learning adventure →</button><div class="cv2-settings"><button data-action="calm" aria-pressed="${!!s.calm}">Gentle motion: ${s.calm?'on':'off'}</button><button data-action="quality" aria-pressed="${s.quality==='low'}">Low power: ${s.quality==='low'?'on':'off'}</button><button data-action="night" aria-pressed="${!!s.night}">${s.night?'Moonlit':'Morning'} scenery</button></div>`;
        }html+='</section>';
    ...
    const HOMES=['tent','trailtent','canvas','cabin','lodge','stonehome','keep'];let movingIn=null;
    ...
    function tap(px,py){if(paused)return;if(s.challenge)return;if(focusedPanel()){panel=null;ui();return;}const p=cell(px,py);if(preview){preview.x=p.x;preview.y=p.y;preview.lastPlaced=false;if(preview.brush)placePreview();else ui();return;}
    ...
    (options.loadScene?options.loadScene():import('./camp-v2-scene.js')).then(module=>{
      ...
      scene=module.createScene(canvas,{catalog,footprint,sources,world,avatar,guardians,placementReason,lowPower:s.quality==='low',
