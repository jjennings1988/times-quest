...
(function (root) {
  ...
  function mount(host,options){
    ...
    return {dispose(){if(disposed)return;persistView();disposed=true;clearTimeout(loadTimer);abort.abort();cancelAnimationFrame(raf);try{scene?.dispose();}catch{};scene=null;host.classList.remove('cv2-three','cv2-no-webgl');host.innerHTML='';}};
