...
(function (root) {
  ...
  function validSave(s){
    ...
    if(s.v===3){
      ...
      if(s.challenge&&(!['fish','stone','tree','land','order'].includes(s.challenge.kind)||!Array.isArray(s.challenge.questions)||!(s.challenge.kind==='order'?[1]:[2,3]).includes(s.challenge.questions.length)||!Number.isInteger(s.challenge.index)||s.challenge.index<0||s.challenge.index>=s.challenge.questions.length||s.challenge.questions.some(q=>!Number.isInteger(q.a)||!Number.isInteger(q.b)||q.a<0||q.a>12||q.b<0||q.b>12)))return false;
    }
  ...
  function command(save,cmd,context={}){
    ...
    if(['place','move','upgrade'].includes(cmd.kind)){
      ...
      if(kind==='order'&&Array.isArray(cmd.questions)&&cmd.questions.length)cmd={...cmd,questions:[cmd.questions.find(q=>q&&q.a>0&&q.b>0)||cmd.questions[0]]};
      if(!Array.isArray(cmd.questions)||!(kind==='order'?[1]:[2,3]).includes(cmd.questions.length)||cmd.questions.some(q=>!Number.isInteger(q.a)||!Number.isInteger(q.b)||q.a<0||q.a>12||q.b<0||q.b>12))return {ok:false,message:'Review questions are unavailable'};
      s.challenge={kind,target:target||null,questions:cmd.questions.map(q=>({a:q.a,b:q.b})),index:0,retry:false,feedback:false};message=kind==='order'?'One order for Mara, at your pace. Mistakes are for learning.':(cmd.questions.length===2?'Two':'Three')+' review facts, at your pace. Mistakes are for learning.';
      ...
      else{s.patternTry=null;s.solved=[...(s.solved||[]).filter(k=>k!==p.key),p.key].slice(-60);const fact=`${p.type}:${Math.min(p.rows,p.cols)}x${Math.max(p.rows,p.cols)}`;
  ...
  function awardLearning(save,gems){if(!validSave(save))return save;const s=clone(migrateSave(save));s.gems+=Math.max(0,Math.floor(Number(gems)||0));s.harvested=[];s.casts=3;s.orders=2;s.wood+=2;if(s.mastered>=3)s.stone+=4;s.revision++;return s;}
  ...
  root.CampV2={reframeCamera,content,buildable,unlockReason,syncProgress,grantKits,autoPlace,patterns,mathable,request,requestMet,world,catalog,COLS,ROWS,sources,fresh,freshExpedition,validSave,migrateSave,footprint,placementReason,route,command,awardLearning,mount};
