...
(function (root) {
  ...
  function unlockReason(s,type){const d=catalog[type];return (s.mastered||0)<(d?.need||0)?'Complete '+d.need+' Mastery Trials to unlock this blueprint':'';}
  ...
  function command(save,cmd,context={}){
    ...
    if(['place','move','upgrade'].includes(cmd.kind)){
      ...
      if(cmd.kind!=='move'&&!(cmd.kind==='place'&&(s.inventory[type]||0)>0)&&unlockReason(s,type))return {ok:false,message:unlockReason(s,type)};
      ...
      if(s.mastered<(offer.need||0))return {ok:false,message:'Complete '+offer.need+' Mastery Trials first'};
  ...
  function mount(host,options){
    ...
    function dispatch(e){const b=e.target.closest('button');if(!b||b.disabled)return;
      ...
      if(a==='close'||a==='cancel'){if(preview?.brush)mode='explore';preview=null;selected=null;panel=null;ui();host.querySelector(`[data-action="${mode}"]`)?.focus();return;}
  ...
  root.CampV2={reframeCamera,content,buildable,unlockReason,syncProgress,world,catalog,COLS,ROWS,sources,fresh,validSave,migrateSave,footprint,placementReason,route,command,awardLearning,mount};
