...
function renderFamilyLesson(){
  ...
  if(!locked){
    controls=`<div class="strategy-actions">${f===5?'<p>Tap any boat to change its dock. Keep its whole cargo together.</p>':f===1&&!done?`<fieldset class="strategy-choices strategy-equations"><legend>Which equation matches this tray before Echo receives it?</legend>${p.equations(s.each).map((t,i)=>`<button data-strategy-focus="eq-${i}" onclick="strategyAction('act',${i})">${s.phase>0?t.split(' =')[0]:t}</button>`).join('')}</fieldset>`:`<button class="strategy-act" data-strategy-focus="act" onclick="strategyAction('act')" ${done?'disabled':''}>${done?'Groups ready':p.actions[s.step]}</button>`}<button data-strategy-focus="undo" onclick="strategyAction('undo')" ${(f===5?!(s.moved.length||s.history.length):!s.step)?'disabled':''}>Undo last change</button></div>`;
