...
(function(root){
  ...
  function groups(catalog){const result={};for(const id of Object.keys(catalog).filter(id=>!catalog[id].kitOnly)){const key=category(catalog,id);(result[key]||=[]).push(id);}return Object.entries(result).map(([name,ids])=>({name,ids:ids.sort((a,b)=>(catalog[a].need||0)-(catalog[b].need||0))}));}
