...
function improveMapNavigation(){
  ...
  controls.innerHTML=`${typeof useChartMap==='function'&&useChartMap()?'<button onclick="MapZoom.step(-1)" aria-label="Zoom out of the map">−</button><button onclick="MapZoom.step(1)" aria-label="Zoom in to the map">+</button>':''}<button onclick="centerExplorer()" aria-label="Find my explorer">⌖</button><button onclick="showRealmList()" aria-label="Choose a realm">Realms</button>${due.length?`<button onclick="startShortReview()">Revisit ${due.length} facts</button>`:''}${resume?'<button onclick="resumeLesson()">Resume lesson</button>':''}`;
}
